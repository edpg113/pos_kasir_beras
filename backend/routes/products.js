...
router.post("/products", (req, res) => {
  ...
  db.query(
    query,
    [namaProduk, kategori, harga, modal, stok, hargaPerKg],
    (err, result) => {
      if (err) {
...
router.put("/editproduct/:id", (req, res) => {
  ...
  db.query(
    query,
    [namaProduk, kategori, harga, modal, stok, hargaPerKg, id],
    (err) => {
...
router.get("/products/export", (req, res) => {
  const query =
    "SELECT namaProduk, kategori, harga, modal, stok, harga_per_kg, is_active FROM produk WHERE is_active = 1 ORDER BY namaProduk ASC";
  db.query(query, (err, result) => {
    if (err) {
    ...
    try {
      ...
      const filename = `Daftar_Produk_${new Date()
        .toISOString()
        .slice(0, 10)}.pdf`;
      ...
      function drawHeader(y) {
        ...
        cols.forEach((c) => {
          ...
          const align =
            c === "namaProduk" || c === "kategori" ? "left" : "right";
      ...
      result.forEach((row) => {
        ...
        const kategoriH = doc.heightOfString(String(row.kategori || ""), {
        ...
        doc.text(String(row.namaProduk || ""), colX.namaProduk, y, {
          width: colWidths.namaProduk,
          align: "left",
        });
        doc.text(String(row.kategori || ""), colX.kategori, y, {
          width: colWidths.kategori,
          align: "left",
        });
        ...
        doc.text(row.stok != null ? String(row.stok) : "-", colX.stok, y, {
